...
const DRINK_OPTIONS = [
  { id: 'glass', amount: 250, label: 'Glass', helper: 'A standard drinking glass.' },
  { id: 'bottle', amount: 500, label: 'Bottle', helper: 'Standard water bottle.' },
  { id: 'large-bottle', amount: 1000, label: 'Large Bottle', helper: 'Extra large bottle.', displayAmount: '1 L' },
  { id: 'jug', amount: 2000, label: 'Jug', helper: 'Large water jug.', displayAmount: '2 L' },
];
...
const WaterIntakeSelector = ({
  ...
  const fillPercent = Math.min(65, Math.max(25, (selectedOption.amount / TANK_CAPACITY_ML) * 100));
  const displayValue = selectedOption.amount >= 1000
    ...
    <View style={styles.container}>
      <Card>
        ...
        <Text style={styles.description}>
          Stay hydrated by tracking your water intake. A daily goal of 2–3 liters is a good
          starting point—adjust based on your activity level and climate.
        </Text>
        ...
        <View style={styles.grid}>
          {DRINK_OPTIONS.map((option) => {
            ...
            const displayVal =
              option.displayAmount || (option.amount >= 1000 ? `${option.amount / 1000} L` : `${option.amount} ml`);
            return (
              <TouchableOpacity
                key={option.id}
              ...
              >
                {isSelected && (
                  <View style={styles.checkWrap}>
                    <Ionicons name="checkmark" size={18} color={colors.havelockBlue} />
                  </View>
                )}
                <Text style={styles.gridAmount}>{displayVal}</Text>
                <Text style={styles.gridLabel}>{option.label}</Text>
                <Text style={styles.gridHelper} numberOfLines={2}>{option.helper}</Text>
              </TouchableOpacity>
            );
          })}
        </View>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.85}
        >
          <Ionicons name="water" size={20} color={colors.white} style={styles.drinkIcon} />
          <Text style={styles.drinkButtonText}>Drink</Text>
        </TouchableOpacity>

        <Text style={styles.tip}>Tip: aim for 6–8 servings spread evenly across your day.</Text>
      </Card>
    </View>
  );
};
...
const styles = StyleSheet.create({
  ...
  cardTitle: { ...typography.h4, color: colors.mineShaft, marginBottom: spacing.sm },
  description: {
  ...
  chooseLabel: { ...typography.label, color: colors.mineShaft, marginBottom: spacing.sm },
  tankWrap: { marginBottom: spacing.md },
  chooseHint: { ...typography.caption, color: colors.raven, marginBottom: spacing.md },
  grid: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: spacing.sm,
  ...
  gridLabel: { ...typography.caption, fontWeight: '600', color: colors.mineShaft, marginBottom: 2 },
